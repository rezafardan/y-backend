export default function validateBlogFields(reqBody: any, status: string): void {
  ...
  const missingFields = requiredFieldsForPublish.filter(
    (field) => status !== "DRAFT" && !reqBody[field]
  );
